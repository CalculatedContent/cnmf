...
G = zeros((nmf_mdl._num_samples, nmf_mdl._num_bases))
            
G[range(len(nmf_mdl.H.T)), nmf_mdl.H.T] = 1.0 
G += 0.01 
num_i = zeros(nmf_mdl._num_bases)                       
G /= tile(reshape(num_i[assign],(-1,1)), G.shape[1])
nmf_mdl.G = G
...
plot_gallery('CNMF seeded' ,nmf_mdl.H)
pl.show()
